...
class RefDeductionSetup extends CORE_Controller
{
    ...
    function __construct() {
        ...
        if($this->session->userdata('right_deductionsetup_view') == 0 || $this->session->userdata('right_deductionsetup_view') == null) {
            redirect('../Dashboard');
        }
        else{
    ...
    function transaction($txn = null) {
        switch ($txn) {
            ...
            case 'getdeductionlist':
                $employee_id = $this->input->post('employee_id', TRUE);
                $pay_period_id = $this->input->post('pay_period_id', TRUE);
                $pay_period_sequence = $this->input->post('pay_period_sequence', TRUE);
                $payperiod=$this->RefDeductionSetup_model->getcycleperiod($pay_period_id);
                $pay_period_sequence = $payperiod[0]->pay_period_sequence;
                $response['data']=$this->RefDeductionSetup_model->getalldeduct($employee_id,$pay_period_id,$pay_period_sequence);

                echo json_encode($response);
                break;

            case 'getdeductionlistedit':
                $employee_id = $this->input->post('employee_id', TRUE);
                $dtr_id = $this->input->post('dtr_id', TRUE);
                $response['data']=$this->RefDeductionSetup_model->getalldeductedit($employee_id,$dtr_id);
           ...
           /* case 'getdtrdeductions':
            ...
            case 'update':
                ...
                $m_deduction->deduction_type_id = $this->input->post('deduction_type_id', TRUE);
                $m_deduction->deduction_desc = $this->input->post('deduction_desc', TRUE);
                $m_deduction->date_modified = date("Y-m-d H:i:s");
                $m_deduction->modified_by = $this->session->user_id;
            ...
            case 'test':
                $pay_period_id=1;
            	$payperiod=$this->RefDeductionSetup_model->getcycleperiod($pay_period_id);
                echo $payperiod[0]->pay_period_sequence;
            	break;
