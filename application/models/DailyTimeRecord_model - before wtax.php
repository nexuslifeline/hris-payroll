...
class DailyTimeRecord_model extends CORE_Model {
    ...
    function Wtax_lookup($taxable_amount) {
                                $semimonthly = $this->db->query('SELECT (CASE  
    WHEN '.$taxable_amount.' BETWEEN reftaxcode.col1 AND reftaxcode.col2-1 THEN (reftaxcode.col2-'.$taxable_amount.')*(ref_payment_type.col2_percent)+(ref_payment_type.col2_amount)
    WHEN '.$taxable_amount.' BETWEEN reftaxcode.col2 AND reftaxcode.col3-1 THEN (reftaxcode.col3-'.$taxable_amount.')*(ref_payment_type.col3_percent)+(ref_payment_type.col3_amount)
    WHEN '.$taxable_amount.' BETWEEN reftaxcode.col3 AND reftaxcode.col4-1 THEN (reftaxcode.col4-'.$taxable_amount.')*(ref_payment_type.col4_percent)+(ref_payment_type.col4_amount)
    WHEN '.$taxable_amount.' BETWEEN reftaxcode.col4 AND reftaxcode.col5-1 THEN (reftaxcode.col5-'.$taxable_amount.')*(ref_payment_type.col5_percent)+(ref_payment_type.col5_amount)
    WHEN '.$taxable_amount.' BETWEEN reftaxcode.col5 AND reftaxcode.col6-1 THEN (reftaxcode.col6-'.$taxable_amount.')*(ref_payment_type.col6_percent)+(ref_payment_type.col6_amount)
    WHEN '.$taxable_amount.' BETWEEN reftaxcode.col6 AND reftaxcode.col7-1 THEN (reftaxcode.col7-'.$taxable_amount.')*(ref_payment_type.col7_percent)+(ref_payment_type.col7_amount)
    WHEN '.$taxable_amount.' BETWEEN reftaxcode.col7 AND reftaxcode.col8-1 THEN (reftaxcode.col8-'.$taxable_amount.')*(ref_payment_type.col8_percent)+(ref_payment_type.col8_amount)
    ELSE ('.$taxable_amount.'-reftaxcode.col8)*(ref_payment_type.col8_percent)+(ref_payment_type.col8_amount) END) as wtax
    FROM reftaxcode
                ...
                foreach($dtr_id as $dtr)
                              {
                                ...
                                if($withholdingtax_stat=="true"){
                                  $data_deductions_withholdingtax[0] = 
                                     array(
                                        'pay_slip_id' => $pay_slip_id,
                                        'deduction_id' => 4,
