                ...
                for($i=0;$i<count($dtr_id);$i++)
                              {
                                ...
                                $regulardeduction = $this->db->query('SELECT new_deductions_regular.deduction_regular_id,deduction_per_pay_amount,daily_time_record.dtr_id,is_deduct,new_deductions_regular.deduction_id FROM daily_time_record
                                                                    ...
                                                                    WHERE daily_time_record.employee_id='.$processtemp[0]->employee_id.' AND deduction_cycle='.$processtemp[0]->pay_period_sequence.' AND is_temporary=0 AND new_deductions_regular.deduction_id!=1
                                ...
                                $tempdeduction = $this->db->query('SELECT new_deductions_regular.deduction_regular_id,deduction_per_pay_amount,daily_time_record.dtr_id,is_deduct,new_deductions_regular.deduction_id FROM daily_time_record
                                                                    ...
                                                                    LEFT JOIN new_deductions_regular
                                                                    ON dtr_deductions.deduction_id=new_deductions_regular.deduction_id
                                                                    WHERE daily_time_record.employee_id='.$processtemp[0]->employee_id.' AND daily_time_record.pay_period_id='.$processtemp[0]->pay_period_id.' AND is_temporary=1 AND new_deductions_regular.deduction_id!=1
                                                                    AND new_deductions_regular.deduction_id!=2 AND new_deductions_regular.deduction_id!=3 AND new_deductions_regular.deduction_id!=4');
                                ...
                                //return true;
                                  ...
                                  //echo "update";
                                $this->db->select('daily_time_record.employee_id,pay_slip.pay_slip_id,
                                sss_phic_salary_credit,philhealth_salary_credit,pagibig_salary_credit,tax_shield,
                                pay_period_sequence,daily_time_record.pay_period_id,
                                reg_amt,sun_amt,reg_hol_amt,
                                sun_reg_hol_amt,sun_reg_hol_amt,
                                ...
                                $this->db->join('emp_rates_duties', 'emp_rates_duties.employee_id = daily_time_record.employee_id', 'left');
                                $this->db->join('refpayperiod', 'refpayperiod.pay_period_id = daily_time_record.pay_period_id', 'left');
                                $this->db->join('pay_slip', 'pay_slip.dtr_id = daily_time_record.dtr_id', 'left');
                                $query = $this->db->get('daily_time_record');
                                $processtemp=$query->result();
                                //echo $processtemp[0]->pay_period_sequence;
                                //PROCESS PAYROLL
                                $pay_slip_id_update = $processtemp[0]->pay_slip_id;
                                $reg_pay = $processtemp[0]->reg_amt;
                                $sun_pay = $processtemp[0]->sun_amt;
                                $reg_hol_pay = $processtemp[0]->reg_hol_amt + $processtemp[0]->sun_reg_hol_amt;
                                $spe_hol_pay = $processtemp[0]->spe_hol_amt + $processtemp[0]->sun_spe_hol_amt;
                                ...
                                foreach ($regearningstemp->result() as $row)
                                { 
                                        $reg_earnings_temp = $row->oe_regular_amount;
                                        $regular_earnings+=$row->oe_regular_amount;
                                        $regearningstempupdate = $this->db->query('SELECT * FROM pay_slip_other_earnings WHERE pay_slip_id='.$processtemp[0]->pay_slip_id.' AND oe_regular_id='.$row->oe_regular_id);
                                           if ($regearningstempupdate->num_rows() == 0) {
                                              //echo "wala";
                                              $datareginsert = array(
                                                      'pay_slip_id' => $processtemp[0]->pay_slip_id,
                                                      'earnings_id' => $row->earnings_id,
                                                      'earnings_amount' => $reg_earnings_temp,
                                                      'oe_regular_id' => $row->oe_regular_id
                                              );
                                              $this->db->insert('pay_slip_other_earnings', $datareginsert);
                                            }
                                            else{
                                              foreach ($regearningstempupdate->result() as $row)
                                              {
                                              //echo "meron";
                                              $this->db->set('earnings_amount', $reg_earnings_temp);
                                              $this->db->where('pay_slip_other_earnings_id', $row->pay_slip_other_earnings_id);
                                              $this->db->update('pay_slip_other_earnings');
                                              }
                                            }
                                        
                                }
                                //echo json_encode($regearningstempupdate->result());
                                 //GET TEMPORARY EARNINGS
                                $temporary_earnings=0;
                                $tempearnings = $this->db->query('SELECT oe_regular_id,earnings_id,oe_regular_amount FROM new_otherearnings_regular WHERE employee_id='.$processtemp[0]->employee_id.' AND pay_period_id='.$processtemp[0]->pay_period_id.' AND is_temporary=1');
                                foreach ($tempearnings->result() as $row)
                                {
                                        $temporary_earnings+=$row->oe_regular_amount;
                                        $temp_earnings_temp = $row->oe_regular_amount;
                                        $tempearningstempupdate = $this->db->query('SELECT * FROM pay_slip_other_earnings WHERE pay_slip_id='.$processtemp[0]->pay_slip_id.' AND oe_regular_id='.$row->oe_regular_id);
                                           //echo json_encode($tempearningstempupdate->result());
                                        //echo $tempearningstempupdate->num_rows();
                                           if ($tempearningstempupdate->num_rows() == 0) {
                                              //echo "wala";
                                              $datareginsert = array(
                                                      'pay_slip_id' => $processtemp[0]->pay_slip_id,
                                                      'earnings_id' => $row->earnings_id,
                                                      'earnings_amount' => $temp_earnings_temp,
                                                      'oe_regular_id' => $row->oe_regular_id
                                              );
                                              $this->db->insert('pay_slip_other_earnings', $datareginsert);
                                            }
                                            else{
                                              foreach ($tempearningstempupdate->result() as $row)
                                              {
                                              //echo "meron";
                                              $this->db->set('earnings_amount', $temp_earnings_temp);
                                              $this->db->where('pay_slip_other_earnings_id', $row->pay_slip_other_earnings_id);
                                              $this->db->update('pay_slip_other_earnings');
                                              }
                                            }
                                }
                                 //GET REGULAR DEDUCTIONS AND UPDATE
                                $regular_deductions=0;
                                $regdeduction = $this->db->query('SELECT deduction_id,deduction_regular_id,deduction_per_pay_amount,deduction_regular_id FROM new_deductions_regular WHERE employee_id='.$processtemp[0]->employee_id.' AND deduction_cycle='.$processtemp[0]->pay_period_sequence.' AND is_temporary=0');
                                //echo json_encode($regdeduction->result());
                                foreach ($regdeduction->result() as $row)
                                {
                                        
                                        $checkifdeduct = $this->db->query('SELECT is_deduct FROM dtr_deductions WHERE dtr_id='.$dtr_id[$i].' AND  deduction_id='.$row->deduction_id);
                                        //echo $row->deduction_id;
                                        //echo json_encode($checkifdeduct->result());
                                        $deduct_stat;
                                        foreach ($checkifdeduct->result() as $ifdeduct)
                                        {
                                            if($ifdeduct->is_deduct==0){
                                                $deduct_stat=FALSE;
                                            }
                                            else{
                                                $deduct_stat=TRUE;
                                                ...
                                                $reg_deduction_temp = $row->deduction_per_pay_amount;
                                                $deduction_id_update=$row->deduction_id;
                                                $regdeductiontempupdate = $this->db->query('SELECT * FROM pay_slip_deductions WHERE pay_slip_id='.$processtemp[0]->pay_slip_id.' AND deduction_regular_id='.$row->deduction_regular_id);
                                                
                                                //echo json_encode($regdeductiontempupdate->result());
                                                    if ($regdeductiontempupdate->num_rows() == 0) {
                                                      //echo "wala";
                                                      $datareginsert = array(
                                                              'pay_slip_id' => $processtemp[0]->pay_slip_id,
                                                              'deduction_id' => $row->deduction_id,
                                                              'deduction_regular_id' => $row->deduction_regular_id,
                                                              'deduction_amount' => $reg_deduction_temp,
                                                              'active_deduct' => $deduct_stat,
                                                      );
                                                      $this->db->insert('pay_slip_deductions', $datareginsert);
                                                    }
                                                    else{
                                                    //echo "meron";
                                                      foreach ($regdeductiontempupdate->result() as $row)
                                                      {
                                                      //echo "meron";
                                                      $this->db->set('deduction_amount', $reg_deduction_temp);
                                                      $this->db->set('active_deduct', $deduct_stat);
                                                      $this->db->set('deduction_id', $deduction_id_update);
                                                      $this->db->where('pay_slip_deduction_id', $row->pay_slip_deduction_id);
                                                      $this->db->update('pay_slip_deductions');
                                                      }
                                                    }


                                }
                                 //GET TEMPORARY DEDUCTIONS AND UPDATE
                                $temporary_deductions=0;
                                $tempdeduction = $this->db->query('SELECT deduction_id,deduction_regular_id,deduction_per_pay_amount,deduction_regular_id FROM new_deductions_regular WHERE employee_id='.$processtemp[0]->employee_id.' AND pay_period_id='.$processtemp[0]->pay_period_id.' AND is_temporary=1');
                                foreach ($tempdeduction->result() as $row)
                                {
                                    ...
                                    $checkifdeduct = $this->db->query('SELECT is_deduct FROM dtr_deductions WHERE dtr_id='.$dtr_id[$i].' AND  deduction_id='.$row->deduction_id);
                                        ...
                                        foreach ($checkifdeduct->result() as $ifdeduct)
                                        {
                                            if($ifdeduct->is_deduct==0){
                                                ...
                                                $temp_deduction_temp = $row->deduction_per_pay_amount;
                                                $deduction_id_update=$row->deduction_id;
                                                $tempdeductiontempupdate = $this->db->query('SELECT * FROM pay_slip_deductions WHERE pay_slip_id='.$processtemp[0]->pay_slip_id.' AND deduction_regular_id='.$row->deduction_regular_id);
                                                
                                                //echo json_encode($regdeductiontempupdate->result());
                                                    if ($tempdeductiontempupdate->num_rows() == 0) {
                                                      //echo "wala";
                                                      $datareginsert = array(
                                                              'pay_slip_id' => $processtemp[0]->pay_slip_id,
                                                              'deduction_id' => $row->deduction_id,
                                                              'deduction_regular_id' => $row->deduction_regular_id,
                                                              'deduction_amount' => $temp_deduction_temp,
                                                              'active_deduct' => $deduct_stat,
                                                      );
                                                      $this->db->insert('pay_slip_deductions', $datareginsert);
                                                    }
                                                    ...
                                                    //echo "meron";
                                                      foreach ($tempdeductiontempupdate->result() as $row)
                                                      {
                                                      //echo "meron";
                                                      $this->db->set('deduction_amount', $temp_deduction_temp);
                                                      $this->db->set('active_deduct', $deduct_stat);
                                                      $this->db->set('deduction_id', $deduction_id_update);
                                                      $this->db->where('pay_slip_deduction_id', $row->pay_slip_deduction_id);
                                                      $this->db->update('pay_slip_deductions');
                                ...
                                //insert or update to pay slip deductions
                                if($sss_stat=="true"){
                                        $temppremiumdeduct = $this->db->query('SELECT * FROM pay_slip_deductions WHERE pay_slip_id='.$processtemp[0]->pay_slip_id.' AND deduction_id=1');
                                            if ($temppremiumdeduct->num_rows() == 0) {
                                                $data_deductions1[] = 
                                                    array(
                                                    'pay_slip_id' => $processtemp[0]->pay_slip_id,
                                                    'deduction_id' => 1,
                                                    ...
                                                    $this->db->insert_batch('pay_slip_deductions', $data_deductions1); 
                                                }
                                            else{
                                                $this->db->set('deduction_amount', $sssdeduct);
                                                $this->db->set('sss_id', $sss_id);
                                                $this->db->set('active_deduct', TRUE);
                                                $this->db->where('pay_slip_id', $processtemp[0]->pay_slip_id);
                                  ...
                                  //amke amount to zero and make inactive
                                    $this->db->set('deduction_amount', $sssdeduct);
                                        $this->db->set('sss_id', null);
                                        $this->db->set('active_deduct', FALSE);
                                        $this->db->where('pay_slip_id', $processtemp[0]->pay_slip_id);
                                        $this->db->where('deduction_id', 1);
                                        $this->db->update('pay_slip_deductions');
                                }
                                ...
                                if($philhealth_stat=="true"){
                                        $temppremium_philhealth = $this->db->query('SELECT * FROM pay_slip_deductions WHERE pay_slip_id='.$processtemp[0]->pay_slip_id.' AND deduction_id=2');
                                            if ($temppremium_philhealth->num_rows() == 0) {
                                                $data_deductions2[] = 
                                                    array(
                                                    'pay_slip_id' => $processtemp[0]->pay_slip_id,
                                                    'deduction_id' => 2,
                                                    ...
                                                    $this->db->insert_batch('pay_slip_deductions', $data_deductions2); 
                                                }
                                            else{
                                                $this->db->set('deduction_amount', $philhealthdeduct);
                                                $this->db->set('philhealth_id', $philhealth_id);
                                                $this->db->set('active_deduct', TRUE);
                                                $this->db->where('pay_slip_id', $processtemp[0]->pay_slip_id);
                                  ...
                                  //make deduct amout to zero
                                        $this->db->set('deduction_amount', $philhealthdeduct);
                                        $this->db->set('philhealth_id', null);
                                        $this->db->set('active_deduct', FALSE);
                                        $this->db->where('pay_slip_id', $processtemp[0]->pay_slip_id);
                                        $this->db->where('deduction_id', 2);
                                        $this->db->update('pay_slip_deductions');
                                }
                                    //pag ibig update or insert
                                if($pagibig_stat=="true"){
                                        $temppremium_pagibig = $this->db->query('SELECT * FROM pay_slip_deductions WHERE pay_slip_id='.$processtemp[0]->pay_slip_id.' AND deduction_id=3');
                                            if ($temppremium_pagibig->num_rows() == 0) {
                                                $data_deductions3[] = 
                                                    array(
                                                    'pay_slip_id' => $processtemp[0]->pay_slip_id,
                                                    'deduction_id' => 3,
                                                    ...
                                                    $this->db->insert_batch('pay_slip_deductions', $data_deductions3); 
                                                }
                                            else{
                                                $this->db->set('deduction_amount', $pagibigdeduct);
                                                $this->db->set('active_deduct', TRUE);
                                                $this->db->where('pay_slip_id', $processtemp[0]->pay_slip_id);
                                    ...
                                    //withholding update or insert
                                if($withholdingtax_stat=="true"){
                                        $temppremiumwithholding = $this->db->query('SELECT * FROM pay_slip_deductions WHERE pay_slip_id='.$processtemp[0]->pay_slip_id.' AND deduction_id=4');
                                            if ($temppremiumwithholding->num_rows() == 0) {
                                                $data_deductions4[] = 
                                                    array(
                                                    'pay_slip_id' => $processtemp[0]->pay_slip_id,
                                                    'deduction_id' => 4,
                                                    ...
                                                    $this->db->insert_batch('pay_slip_deductions', $data_deductions4); 
                                                }
                                            else{
                                                $this->db->set('deduction_amount', $taxshielddeduct);
                                                $this->db->set('active_deduct', TRUE);
                                                $this->db->where('pay_slip_id', $processtemp[0]->pay_slip_id);
