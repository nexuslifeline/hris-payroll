...
class DailyTimeRecord_model extends CORE_Model {
    ...
    function Philhealth_lookup_shield($philhealthlookuptaxshield) {
      $ssstemplookup=$this->db->query('SELECT ref_philhealth_contribution_id,employee FROM ref_philhealth_contribution WHERE '.$philhealthlookuptaxshield.' BETWEEN salary_range_from AND salary_range_to');
                                        $ssstempreflookup = $ssstemplookup->result();
                                        return $ssstempreflookup[0]->employee;
                ...
                foreach($dtr_id as $dtr)
                              {
                                ...
                                $checkifexists = $this->db->query('SELECT dtr_id,pay_slip_id,pay_slip_code FROM pay_slip WHERE dtr_id='.$dtr);
                                ...
                                //
                                  ...
                                  if($processtemp[0]->pay_period_sequence==$philhealth_check1 AND $philhealth_is_deduct==1){
                                    ...
                                    if($processtemp[0]->philhealth_salary_credit==0.00 || null)
                                    {
                                        ...
                                        $philhealthdeduct=$this->Philhealth_lookup_shield($philhealthlookuptaxshield);
                                        $philhealthdeduct = $this->verify_half_deduct($verify_amount_value = $philhealthdeduct,$ref_payment_type_id_lookup);
                                        $philhealth_id=0;
                                        /*echo $philhealthdeduct;*/
                                    }
                                  }
                                  //CYCLE 2
                                ...
                                $regulardeduction = $this->db->query('SELECT new_deductions_regular.deduction_regular_id,deduction_per_pay_amount,daily_time_record.dtr_id,is_deduct,new_deductions_regular.deduction_id FROM daily_time_record
                                                                    ...
                                                                    AND new_deductions_regular.deduction_id!=7 AND new_deductions_regular.deduction_id!=8 AND new_deductions_regular.deduction_id!=9
                                ...
                                $regulardeductionloans = $this->db->query('SELECT new_deductions_regular.deduction_regular_id,deduction_per_pay_amount,daily_time_record.dtr_id,is_deduct,new_deductions_regular.deduction_id FROM daily_time_record
                                                                    LEFT JOIN dtr_deductions
                                                                    ON daily_time_record.dtr_id=dtr_deductions.dtr_id
                                                                    LEFT JOIN new_deductions_regular
                                                                    ON dtr_deductions.deduction_id=new_deductions_regular.deduction_id
                                                                    WHERE new_deductions_regular.employee_id='.$processtemp[0]->employee_id.' AND daily_time_record.dtr_id='.$dtr.' AND is_temporary=0
                                                                    AND (new_deductions_regular.deduction_id=7 OR new_deductions_regular.deduction_id=8 OR new_deductions_regular.deduction_id=9)
                                                                    AND new_deductions_regular.is_deleted=0');
                                 ...
                                 /* $m_products->set('quantity','quantity-'.$invalue);*/
                                    ...
                                    for($z_deduct_2;$rd>$z_deduct_2;$z_deduct_2++){
                                            $pay_slip_deduct_temp = $this->db->query('SELECT ndr.loan_total_amount,deduction_total_amount,pay_slip_deductions.deduction_regular_id,SUM(deduction_amount) as total_loan_deductamount FROM pay_slip_deductions
                                                                        ...
                                                                        WHERE pay_slip_deductions.deduction_regular_id='.$deduction_regular_id[$z_deduct_2]);
                                            $pay_slip_deduct_filter = $pay_slip_deduct_temp->result();
                                            $loan_total_amount = $pay_slip_deduct_filter[0]->loan_total_amount;
